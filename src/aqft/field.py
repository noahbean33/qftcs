...
class ScalarField(FieldOperator):
    ...
    def solve_radial_equation(self, omega, l, r_start, r_end, initial_conditions, num_points=1000):
        """
        Numerically solves the radial part of the Klein-Gordon equation.

        This method assumes the spacetime is static and spherically symmetric,
        allowing for separation of variables. The field is decomposed as:
        phi(t, r, theta, phi) = exp(-i*omega*t) * (R(r)/r) * Y_lm(theta, phi)

        This leads to a second-order ODE for the radial function R(r), which
        is solved here.

        Parameters:
            omega (float): The frequency of the mode.
            l (int): The angular momentum quantum number.
            r_start (float): The starting radial coordinate for integration.
            r_end (float): The ending radial coordinate for integration.
            initial_conditions (tuple): A tuple (R_initial, dR_dr_initial) for R(r) at r_start.
            num_points (int): The number of points to evaluate the solution at.

        Returns:
            tuple: A tuple (r_vals, R_vals) containing the radial points and
                   the corresponding solution R(r).
        """
        if self.spacetime.name != 'Schwarzschild':
            raise NotImplementedError("Numerical solver is currently only implemented for Schwarzschild spacetime.")

        M = self.spacetime.M
        m_field = self.mass

        if r_start <= 2 * M:
            raise ValueError(f"Starting radius r_start={r_start} must be outside the event horizon (r > 2M = {2*M}).")

        # For Schwarzschild spacetime, the Ricci scalar is zero, so the xi coupling term vanishes.
        r = sympy.symbols('r', real=True, positive=True)
        f = 1 - 2 * M / r
        f_prime = sympy.diff(f, r)

        f_num = sympy.lambdify(r, f, 'numpy')
        f_prime_num = sympy.lambdify(r, f_prime, 'numpy')

        # The radial ODE is: R'' + (f'/f)R' + (omega^2/f^2 - (l(l+1)/r^2 + m^2)/f - f'/(f*r))R = 0
        # We convert this to a system of first-order ODEs: y' = [y[1], R'']
        # where y = [R, R']
        def ode_system(r_val, y):
            R_val, dR_dr_val = y
            f_val = f_num(r_val)
            f_prime_val = f_prime_num(r_val)

            if np.isclose(f_val, 0):
                return [np.nan, np.nan]

            # Coefficient of R' is (f'/f)
            coeff_R_prime = f_prime_val / f_val

            # Coefficient of R is (omega^2/f^2 - (l(l+1)/r^2 + m^2)/f - f'/(f*r))
            term1 = omega**2 / f_val**2
            term2 = (l * (l + 1) / r_val**2 + m_field**2) / f_val
            term3 = f_prime_val / (f_val * r_val)
            coeff_R = term1 - term2 - term3

            # R'' = -coeff_R_prime * R' - coeff_R * R
            dR2_dr2_val = -coeff_R_prime * dR_dr_val - coeff_R * R_val
            
            return [dR_dr_val, dR2_dr2_val]

        r_span = [r_start, r_end]
        r_eval = np.linspace(r_start, r_end, num_points)
        ...
        if not solution.success:
            print(f"Warning: ODE integration failed with message: {solution.message}")
